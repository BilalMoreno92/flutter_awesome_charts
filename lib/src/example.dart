    ...
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(
          ...
          title: const Text("Line chart animation demo"),
        ),
        body: Padding(
...
class LineData {
  ...
  LineData(this.category, this.color, List<double> values) {
    data = createCumulativeData(values);
  }
    ...
    return TweenAnimationBuilder(
        ...
        builder: (BuildContext context, double percentage, Widget? child) {
          return CustomPaint(
            ...
            child: const SizedBox(width: double.infinity, height: 340),
          );
        });
  }
}
...
class LineChartPainter extends CustomPainter {
  ...
  void addHorizontalLinesAndSizes(Size size, Canvas canvas) {
    for (int i = 1; i <= 5; i++) {
      ...
      tp.paint(canvas,
          Offset(marginLeft - tp.width - emptySpace, y - emptySpace));
